        ...
        try {
            ...
            if ($strquery !== null) {
                if (trim($strquery) && strlen($strquery) > 2) {
                    $condition .= "and (p.perIdentificacion like '%{$strquery}%' || p.perNombre like '%{$strquery}%' || p.perApellido like '%{$strquery}%' || p.perCorreo like '%{$strquery}%' || p.perTelefono like '%{$strquery}%')";
                }
            }
